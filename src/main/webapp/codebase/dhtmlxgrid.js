...
function dhtmlXGridObject(id) {
    ...
    //patch - allow selections in elements with canselect attribute set - replace the old function (always return false)
    this.entBox.onselectstart = function() {
        var ev = (arguments[0] || event);
        var el = Event.element(ev);
        return !Object.isUndefined(el.canselect);
        //    return false
    };
    ...
    this.doOnScroll = function(e, mode) {
        ...
        if (this._srnd) {
            ...
            this._dLoadTimer = window.setTimeout(function() {
                ...
                // контроль помеченных строчек
                self._checkboxesSelectionControl();
            }, 100)
        }
    };
...
// Не фиксируем жестко ширину грид-контейнера                
...
// Не фиксируем жестко ширину грид-контейнера, для дочерних гридов
//            if (this._delta_x) {
...
// Не фиксируем жестко ширину грид-контейнера
// теперь он сможет занять все свободное по ширине пространство
// т.е. сможет растягиваться и сужаться при изменении размеров окна браузера        
//        this.entBox.style.width = newWidth + (quirks ? 2 : 0) * outerBorder + this._ff_size_delta + "px";
    ...
    this.startColResize = function(ev) {
        ...
        if (el.tagName == "TD" && el.style.cursor != "default") {
            if ((this._drsclmn) && (!this._drsclmn[el._cellIndex]))
                return;
            self._old_d_mm = document.onmousemove;
            self._old_d_mu = document.onmouseup;
    ...
    this.stopColResize = function() {
        document.onmousemove = self._old_d_mm;
        document.onmouseup = self._old_d_mu;
        this.setSizes();
        this.doOnScroll(0, 1)
    ...
    this.doColResize = function(ev, el, startW, x, tabW) {
        ...
        if(ev.clientX >= this.objBox.offsetWidth + this.objBox.parentElement.offsetLeft - 2) {
            self.stopColResize();
            return;
        }
    ...
    //patch
    this._checkRowVisible = function(ind) {
        var r = this.render_row(ind);
        if (!r || r == -1)return false;
        if (r && r.style.display == "none")return false;
        return true;
    };
    this._nextRowInd = function(ind, dir) {
        var r = this.render_row(ind + dir);
        if (!r || r == -1)return null;
        if (r && r.style.display == "none")return this._nextRowInd(ind + dir, dir);
        return ind + dir;
    };
    //end of patch

    //function rewritten to support hidden lines in static grids
    this.scrollPage = function(dir) {
        var master = this._realfake ? this._fake : this;
        var delta = Math.floor(this.objBox.offsetHeight / (this._srdh || 20)) * dir;
        var base = master._r_select || this.getRowIndex(this.row.idd) || 0;
        var new_ind = Math.floor(base + delta);
        ...
        if (this._srnd && !this.rowsBuffer[new_ind]) {
            ...
            //для статических гридов движемся по одной видимой строке
            var cnt = Math.abs(delta);
            new_ind = base;
            var t_ind;
            while (true) {
                t_ind = this._nextRowInd(new_ind, dir);
                if (t_ind == null)break;
                new_ind = t_ind
                        --
                cnt;
    ...
    //patch - select previously selected cell, if it was. Not the first one
    this.selectRow = function(r, fl, preserve, show) {
        if (Object.isUndefined(this._selectedCellIndex))
            var ind = this.getSelectedCellIndex();
        else {
            ind = this._selectedCellIndex;
            this._selectedCellIndex = undefined;
        }
        if (typeof (r) != 'object')
            r = this.render_row(r);
        this.selectCell(r, ind == -1 ? 0 : ind, fl, preserve, false, show)
    };
    ...
    this._fixAlterCss = function(ind) {
        if (this._cssSP && this._h2)return this._fixAlterCssTGR(ind);
    ...
    this.getChangedRows = function(and_added) {
        ...
        this.forEachRow(function(id) {
            ...
            var cols = row.childNodes.length;
            if (and_added && row._added)res[res.length] = row.idd; else
                for (var j = 0; j < cols; j++)if (row.childNodes[j].wasChanged) {
        ...
        for (var jj = 0; jj < this._cCount; jj++) {
            if ((!this._srClmn) || (this._srClmn[jj])) {
                ...
                if (this._chAttr) {
                    ...
                    changeFl = true;
                if (this._sAll && this.cellType[jj] == "tree")out.push((this._h2 ? (" image='" + this._h2.get[r.idd].image + "'") : "") + ">" + zxVal + "</cell>"); else
                    out.push(">" + zxVal + "</cell>");
    ...
    this.serialize = function() {
        ...
        if (this._mathSerialization)this._agetm = "getMathValue"; else
    ...
    this.obj.ondblclick = function(e) {
        if (!this.grid.wasDblClicked(e || window.event))
            return false;
        if (this.grid._dclE)this.grid.editCell(e || window.event);
        (e || event).cancelBubble = true;
...
dhtmlXGridObject.prototype = {getRowAttribute: function(id, name) {
    ...
    setColumnHidden:function(ind, state) {
        ...
        var f = convertStringToBoolean(state);
        if (f) {
            if (!this._hrrar)this._hrrar = new Array(); else if (this._hrrar[ind])return;
    ...
    _parseHead:function(xmlDoc) {
        ...
        if (hheadCol.length) {
            ...
            var split = false;
            if (asettings[0]) {
                for (var s = 0; s < asettings[0].childNodes.length; s++)switch (asettings[0].childNodes[s].tagName) {case "colwidth":
    ...
    _drawTooltip:function(e) {
        ...
        if (ced)el.title = ced.cell._attrs['title']
                || (ced.getTitle
                ? ced.getTitle()
                : (ced.getValue() || "").toString()/*.replace(/<[^>]*>/gi, "") patch убран replace*/);
        return true
    },
    ...
    setColspan:function(row_id, col_ind, colspan) {
        ...
        if ((r._childIndexes) && (r.childNodes[r._childIndexes[col_ind]])) {
            ...
            if ((m) && (m != 1))
                for (var i = 1; i < m; i++) {
                    ...
                    if (n.nextSibling)r.insertBefore(c, n.nextSibling); else
                        r.appendChild(c);
        ...
        if ((colspan) && (colspan > 1)) {
            if (r._childIndexes)var j = r._childIndexes[col_ind]; else {
                var j = col_ind;
    ...
    setColumnExcellType:function(colIndex, type) {
        ...
        if (this.cellType[colIndex] == "math")this._strangeParams[i] = type; else
...
//    },
    ...
    load:function(url, call, type) {
        ...
        this.xmlLoader.onloadAction = function(that, b, c, d, xml) {
            ...
            try {
                if (type == "json") {
                    if (xml && xml.xmlDoc) {
                        if(xml.xmlDoc.readyState == 4 &&
                            (xml.xmlDoc.status == 12029 || xml.xmlDoc.status == 0)){
                            // это означает, что сервер не доступен 12029 - IE, 0 - Хром
                            if (alameda._needsRelogin({need_relogin:true})) return;
                        }
                        eval("data=" + xml.xmlDoc.responseText + ";");
                    } else if (typeof xml == "string") {
                        eval("data=" + xml + ";");
                    }
                    if (data) {                        
                        if (alameda._needsRelogin(data)) {
                            return;
                        } else if (data["query_cache_id"] != undefined && data["query_cache_id"] != null) {
                            // надо ожидать
                            setTimeout(function() {
                                that.xmlLoader.loadXML(url + "&query_cache_id=" + data["query_cache_id"]);
                            }.bind(this), 10000);
                        } else {
                            xml = that["_process_" + type](xml);
                            if (!that._contextCallTimer)that.callEvent("onXLE", [that,0,0,xml]);
                ...
                // иногда тут стреляет по неясным причинам - надо исслеловать
                var addInfo = (xml && xml.xmlDoc) ? xml.xmlDoc.responseText : "";
                alert("Ошибка во время загрузки грида: " + e.message + "\n" + addInfo);
            }
        };
        this.xmlLoader.loadXML(url);
    },
    ...
    sortRows:function(col, type, order) {
        ...
        if (this.isTreeGrid())
            ...
            if (col == -1) { // patch: если колонка - 1, то сортируем по ID
                for (var i = 0; i < this.rowsBuffer.length; i++)arrTS[this.rowsBuffer[i].idd] = this.rowsBuffer[i].idd;
            } else {
