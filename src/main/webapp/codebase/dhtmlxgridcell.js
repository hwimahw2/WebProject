...
function dhtmlXGridCellObject(obj) {
    ...
    this.getValue = function () {
        if ((this.cell.firstChild) && (this.cell.firstChild.tagName == "TEXTAREA"))
            ...
            return (this.cell.innerText || this.cell.innerHTML)._dhx_trim()
        /*patch - innerHTML changed by innerText*/
    };
...
function eXcell_ed(cell) {
    ...
    this.edit = function () {
        ...
        var height = (this.cell.offsetHeight - (_isIE ? 4 : 4));
        this.obj.style.height = ((height < 0) ? 0 : height) + "px";
        /*patch - added check for a negative height value*/
...
eXcell_ch.prototype.setValue = function (val) {
    ...
    var obj = this;
    //patch - some speedup + support for disabled checkboxes (d_extra_ch setDisabledCheckboxes)
    var img = document.createElement('IMG');

    if (Object.isUndefined(this.grid._disableCheckBoxes)) {
        img.src = this.grid.imgURL + "item_chk" + val + ".gif";
        img.onclick = function () {
            new eXcell_ch(this.parentNode).changeState();
            (arguments[0] || event).cancelBubble = true;
        };
    } else {
        if (this.cell._cellIndex == this.grid.flagpos)
            img.onclick = function () {
                new eXcell_ch(this.parentNode).changeState();
                (arguments[0] || event).cancelBubble = true;
            };
    ...
    if (this.cell.hasChildNodes())
        this.cell.replaceChild(img, this.cell.childNodes[0]);
    else
...
function eXcell_ro(cell) {
    ...
    this.getValue = function () {
        return this.cell._clearCell ? "" : (this.cell.innerText || this.cell.innerHTML.toString()._dhx_trim())
    }
};
...
function eXcell_rotxt(cell) {
    ...
    this.setValue = function (val) {
        if (!val || val.toString()._dhx_trim() == "") {
            ...
            this.cell._clearCell = false;
        this.cell._brval = val.toString()._dhx_trim();
        if ((!this.grid.multiLine))
