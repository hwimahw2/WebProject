 ...
 * TODO
 * Patch:
 * editControl option have been addded - check the code for its occurences
 */
...
dhtmlxDblCalendarObject.prototype.setHeader = function(isVisible, isDrag,
		btnsOpt) {
...
dhtmlxDblCalendarObject.prototype.loadUserLanguage = function(lang,
		userCBfunction) {
	this.numLoaded = 0;
	if (userCBfunction)
...
dhtmlxDblCalendarObject.prototype.minimize = function() {
	...
	tr.parentNode.parentNode.style.width = parseInt(tr.parentNode.parentNode.offsetWidth)
			+ 'px';
	if (tr)
...
function dhtmlxCalendarObject(contId, isAutoDraw, options) {
	...
	// patch - following line is added
	Event.observe(document, 'mousedown', function(e) {
		if (this.parent.style.display == '')
			if (this.outsideClick(e, this.entObj))
				this.hide();
	}.bind(this));
...
// patch - close calendar if clicked outside of it
dhtmlxCalendarObject.prototype.outsideClick = function(_event, _element) {
	var mousePos = this.getMousePos(_event);
	var elementData = this.getPos(_element);
	return (mousePos.x > elementData.left && mousePos.x < (elementData.left + elementData.width))
			&& (mousePos.y > elementData.top && mousePos.y < (elementData.top + elementData.height)) ? false
			: true;
}
// patch
dhtmlxCalendarObject.prototype.getMousePos = function(e) {
	var posx = 0;
	var posy = 0;
	if (!e)
		e = window.event;
...
// patch
dhtmlxCalendarObject.prototype.getPos = function(_element) {
	var left_top = Element.cumulativeOffset(_element);
	var w_h = Element.getDimensions(_element);
	return {
		left : left_top.left,
		top : left_top.top,
		width : w_h.width,
...
dhtmlxCalendarObject.prototype.createStructure = function() {
	...
	if (this.con.nodeName == 'INPUT') {
		...
		this.con._c = this.parent;/* patched func */
		this.doOnClick = function(date) {
			if (self.options.editControl == undefined)
				self.con.value = self.getFormatedDate(self.options.dateformat,
						date)
						+ (self.time ? " " + self.getFormatedTime() : "");
			else
				$(self.options.editControl).value = self.getFormatedDate(
						self.options.dateformat, date)
						+ (self.time ? " " + self.getFormatedTime() : "");
			self.hide();
			self.con.focus();
			return true
	...
	if (_isIE) {
		...
		if (this.ifr == undefined && this._dblC == undefined) {
			...
			this.ifr.className = this.style + (this.skinName ? '_' : "")
					+ this.skinName + "_ifr";
...
dhtmlxCalendarObject.prototype.drawHeader = function() {
	...
	if (this.options.headerButtons.indexOf('C') >= 0) {
		...
		btnClear.setAttribute('title', this.options.msgClear);/* patched func */
		btnClear.onmousedown = function(e) {
			...
			if (self.options.editControl == undefined)
				...
				$(self.options.editControl).value = "";
			self.hide()
		};
...
dhtmlxCalendarObject.prototype.drawMonth = function() {
	...
	this.planeMonth.appendChild(document
			.createTextNode(this.options.monthesFNames[this.date.getMonth()]));
	...
	this.planeYear
			.appendChild(document.createTextNode(this.date.getFullYear()));
...
dhtmlxCalendarObject.prototype.drawDays = function() {
	...
	if (!this.daysPan.childNodes.length) {
		for ( var weekNumber = 0; weekNumber < 6; weekNumber++) {
			...
			for ( var i = 0; i < 7; i++) {
				...
				this.daysCells[weekNumber][i].appendChild(document
						.createTextNode(""))
			}
		}
	}
	...
	for ( var weekNumber = 0; weekNumber < 6; weekNumber++) {
		for ( var i = 0; i < 7; i++) {
			...
			cell.setAttribute('id', this.uid + tempDate.getFullYear()
			...
			if (this.insensitiveDates) {
				...
				for ( var j = 0; j < this.insensitiveDates.length; j++) {
					...
					if (s)
						var f = (this.insensitiveDates[j].split(s).length == 2 ? '%m'
								+ s + '%d'
								: '%Y' + s + '%m' + s + '%d');
					if ((s && this.getFormatedDate(f, tempDate) == this.insensitiveDates[j])
							|| (this.insensitiveDates[j].length == 1 && tempDate
									.getDay() == this.insensitiveDates[j])) {
			...
			if (this.sensitiveFrom && this.sensitiveFrom instanceof Array) {
				...
				for ( var j = 0; j < this.sensitiveFrom.length; j++) {
					var s = this.sensitiveFrom[j].indexOf('.') != -1 ? '.'
							: '-';
					var f = (this.sensitiveFrom[j].split(s).length == 2 ? '%m'
							+ s + '%d' : '%Y' + s + '%m' + s + '%d');
			...
			;
			if (this.sensitiveFrom
					&& (tempDate.valueOf() < this.sensitiveFrom.valueOf())) {
				this.addClass(cell, "insensitive");
				tempDate.setDate(tempDate.getDate() + 1);
				continue
			}
			;
			if (this.sensitiveTo
					&& (tempDate.valueOf() > this.sensitiveTo.valueOf())) {
				this.addClass(cell, "insensitive");
				tempDate.setDate(tempDate.getDate() + 1);
				continue
			}
			...
			if (this.holidays)
				for ( var j = 0; j < this.holidays.length; j++) {
					...
					var f = (this.holidays[j].split(s).length == 2 ? '%m' + s
							+ '%d' : '%Y' + s + '%m' + s + '%d');
			...
			cell.onclick = function() {
				if (!self.doOnClick || self.doOnClick(this.thisdate)) {
					...
					if (self.activeCell)
						self.resetClass(self.activeCell);
...
dhtmlxCalendarObject.prototype.onDrag = function(e) {
	...
	if ((e.button === 0) || (e.button === 1)) {
		...
		if (this.time) {
			this.tp.setPosition(getAbsoluteLeft(cal.parent) + 30,
					getAbsoluteTop(cal.parent) + 160)
		}
		;
		if (_isIE) {
...
dhtmlxCalendarObject.prototype.stopDrag = function(e) {
	e = e || event;
	document.body.onmouseup = (this.bu_onmouseup === window.undefined) ? null
			: this.bu_onmouseup;
	document.body.onmousemove = (this.bu_onmousemove === window.undefined) ? null
			: this.bu_onmousemove;
	this.dragging = false
};
...
dhtmlxCalendarObject.prototype.loadUserLanguage = function(language,
		userCBfunction) {
	if (userCBfunction)
		this.onLanguageLoaded = userCBfunction;
	if (!language) {
...
dhtmlxCalendarObject.prototype.show = function() {
	...
	if (this.con.nodeName == 'INPUT') {
		var aleft = getAbsoluteLeft(this.con);
		var atop = getAbsoluteTop(this.con);
		this.parent.style.left = getAbsoluteLeft(this.con)
				+ this.con.offsetWidth + 'px';
...
dhtmlxCalendarObject.prototype.setSkin = function(newSkin) {
	...
	var mode = "";
	mode = (this.minimized ? "_mini" : (this.time ? "_long"
			: (this.options.isWinHeader ? "_maxi" : "")));
	this.entObj.className = this.style + (newSkin ? '_' + newSkin : '');
	if (mode)
		this.entObj.className += " " + this.entObj.className + mode;
	if (this.ifr != undefined) {
		this.ifr.className = this.style + (newSkin ? '_' + newSkin : '') + mode
				+ "_ifr"
	}
	;
	if (this.time)
...
dhtmlxCalendarObject.prototype.nextMonth = function() {
	...
	if (this.checkEvent("onChangeMonth"))
		this.callEvent("onChangeMonth", [
				(this.date.getMonth() + 1 > 12 ? 1 : this.date.getMonth() + 1),
				this.date.getMonth() == 0 ? 12 : this.date.getMonth() ]);
	if (this.isAutoDraw)
		this.draw()
};
dhtmlxCalendarObject.prototype.prevMonth = function() {
	...
	this.date.setMonth(this.date.getMonth() - 1);
	if (this.checkEvent("onChangeMonth"))
		this.callEvent("onChangeMonth", [ this.date.getMonth() + 1,
				this.date.getMonth() + 2 > 12 ? 1 : this.date.getMonth() + 2 ]);
	if (this.isAutoDraw)
		this.draw()
};
	...
	for ( var i = 0; i < dateformat.length; i++) {
		...
		if (plain) {
			...
			case 'j':
				...
				replStr = Math.ceil((date.valueOf() - x.valueOf()) / 1000 / 60
						/ 60 / 24 - 1);
...
dhtmlxCalendarObject.prototype.setFormatedDate = function(dateformatarg, date) {
	...
	function parseMonth(val) {
		var tmpAr = new Array(this.options.monthesSNames,
				this.options.monthesFNames);
...
function dhtmlxRichSelector(parametres) {
	...
	if (this.valueList)
		for ( var i = 0; i < this.valueList.length; i++)
			this.selector.options[i] = new Option(this.titleList[i],
					this.valueList[i], false, false);
...
dhtmlxRichSelector.prototype.show = function() {
	...
	;
	this.selector.selectedIndex = this.selector
			.getIndexByValue(this.activeValue);
	this.editor.focus()
};
...
dhtmlxRichSelector.prototype.onKeyUp = function(e) {
	...
	case 27:
		...
		self.selector.selectedIndex = self.selector.getIndexByValue(
				self.initValue, true);
		self.editor.blur();
		break;
	default:
