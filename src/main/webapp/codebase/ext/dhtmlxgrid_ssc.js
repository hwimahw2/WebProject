...
dhtmlXGridObject.prototype.loadOpenStates = function (name, cookie_param) {
	...
	if (!val) return;
	val = val.split("|");
	for (var i = 0; i < val.length; i++) this.openItem(val[i])
};
...
dhtmlXGridObject.prototype.saveSizeToCookie = function (name, cookie_param) {
	if (this.cellWidthType == 'px') var z = this.cellWidthPX.join(",");
	else var z = this.cellWidthPC.join(",");
...
dhtmlXGridObject.prototype.saveHiddenColumnsToCookie = function (name, cookie_param) {
	...
	if (this._fake && this._fake._hrrar) for (var i = 0; i < this._fake._cCount; i++) hs[i] = this._fake._hrrar[i] ? "1" : "";
...
dhtmlXGridObject.prototype.loadHiddenColumnsFromCookie = function (name) {
	...
	for (var i = 0; i < ar.length; i++) this.setColumnHidden(i, (ar[i] ? true : false))
};
...
dhtmlXGridObject.prototype.saveOrderToCookie = function (name, cookie_param) {
	if (!this._c_order) {
		this._c_order = [];
		var l = this._cCount;
		for (var i = 0; i < l; i++) this._c_order[i] = i
	};
...
dhtmlXGridObject.prototype.loadOrderFromCookie = function (name) {
	...
	if (z.length > 1 && z.length <= this._cCount) {
		for (var i = 0; i < z.length; i++) if ((!this._c_order && z[i] != i) || (this._c_order && z[i] != this._c_order[i])) {
			var t = z[i];
			if (this._c_order) for (var j = 0; j < this._c_order.length; j++) {
...
dhtmlXGridObject.prototype.loadSizeFromCookie = function (name) {
	...
	if (z) this.initCellWidth = z.split(",");
	var z = this._getCookie(name, 0);
	if ((z) && (z.length)) {
		if (!this._fake && this._hrrar) for (var i = 0; i < z.length; i++) if (this._hrrar[i]) z[i] = 0;
		if (this.cellWidthType == 'px') this.cellWidthPX = z.split(",");
		else this.cellWidthPC = z.split(",")
	};
	this.setSizes();
	return true
};
