...
dhtmlXGridObject.prototype.loadCSVFile = function(path,afterCall){this.load(path,afterCall,"csv")
 ...
 if (temp!="")out.push(temp)};return out.join(this.csv.row)};

//patch - function added
dhtmlXGridObject.prototype.__getColumnLabel=function(cin, ind){var z = this.hdr.rows[(ind||0)+1];var n = z.cells[z._childIndexes ? z._childIndexes[parseInt(cin)] : cin];return(_isIE ? n.innerText : n.textContent)};
...
dhtmlXGridObject.prototype._serializeRowToCVS = function(r,i,start,end){
    ...
    for(var jj=start;jj<end;jj++){
        if (this._hiddenColPos.indexOf(jj) != -1) continue;
        header.push(this.__getColumnLabel(jj));
    }
    //end of patch
    var ind=start;
    ...
    for(var jj=start;ind<end;jj++){
        ...
        if ((!this._srClmn)||(this._srClmn[real_ind])){
            /*patch Mike support for array initialized in SetColumnsInfo - one next line*/
            if (this._hiddenColPos.indexOf2(jj) != -1) continue;
            ...
            if ( this._ecspn && cvx.colSpan && cvx.colSpan >1 ){
                cvx=cvx.colSpan-1;for (var u=0;u<cvx;u++)out[out.length] = "";ind++
            }
        }
        else ind++
    };
    if ((this._onlChAttr)&&(!changeFl)) return "";
    return /*patch start*/header.join(this.csv.cell) + '\n' + /*patch end*/out.join(this.csv.cell)
};
