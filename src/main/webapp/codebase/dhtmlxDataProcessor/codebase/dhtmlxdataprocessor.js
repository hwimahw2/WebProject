...
dataProcessor.prototype = {setTransactionMode:function(mode, total) {
    ...
    markRow:function(id, state, mode) {
        ...
        if (this.callEvent("onRowMark", [id,state,mode,invalid])) {
            if (state)str += this.styles[mode]; else
    ...
    _sendData:function(a1, rowId) {
        ...
        if (this._tMode != "POST")a2.loadXML(a3 + ((a3.indexOf("?") != -1) ? "&" : "?") + a1); else
    ...
    _getRowData:function(rowId, pref) {
        ...
        if (this.obj.mytype == "tree") {
            ...
            for (var i = 0; i < this.obj._cCount; i++) {
                ...
                if (this._endnm)str += "&" + pref + this.obj.getColumnId(i) + "=" + this.escape(c.getValue()); else
                    str += "&" + pref + "c" + i_c + "=" + this.escape(c.getValue())
            }
            ;
            var data = this.obj.UserData[rowId];
            if (data) {
                for (var j = 0; j < data.keys.length; j++)if (data.keys[j].indexOf("__") != 0)
