...
dataProcessor.prototype._createConsole = function() {
    ...
    if (document.body)document.body.insertBefore(c, document.body.firstChild); else dhtmlxEvent(window, "load", function() {
...
dataProcessor.prototype._log = function(data) {
    ...
    if (window.console && window.console.log)window.console.log("DataProcessor :: " + data.replace("&nbsp;", " ").replace("<b>", "").replace("</b>", ""))
};
dataProcessor.prototype._updateStat = function(data) {
    var data = ["&nbsp;Current state"];
    for (var i = 0; i < this.updatedRows.length; i++)data.push("&nbsp;ID:" + this.updatedRows[i] + " Status: " + (this.obj.getUserData(this.updatedRows[i], "!nativeeditor_status") || "updated") + ", " + (this.is_invalid(this.updatedRows[i]) || "valid"))
...
dataProcessor.prototype.xml_analize = function(xml) {
    if (_isFF) {
        if (!xml.xmlDoc.responseXML)this._error("Not an XML, probably incorrect content type specified ( must be text/xml ), or some text output was started before XML data"); else if (xml.xmlDoc.responseXML.firstChild.tagName == "parsererror")this._error(xml.xmlDoc.responseXML.firstChild.textContent); else return true
    } else if (_isIE) {
...
dataProcessor.wrap("sendData", function(rowId) {
    if (rowId) {
        ...
        if (this.obj.mytype == "tree") {
            if (!this.obj._idpull[rowId])this._log("&nbsp;Error! item with such ID not exists <b>" + rowId + "</b>")
        } else {
            if (!this.obj.rowsAr[rowId])this._log("&nbsp;Error! row with such ID not exists <b>" + rowId + "</b>")
        }
    }
}, function() {
});
...
dataProcessor.wrap("_sendData", function(url, rowId) {
    if (rowId)this._log("&nbsp;Sending in one-by-one mode, current ID = " + rowId); else
...
dataProcessor.wrap("afterUpdate", function(that, b, c, d, xml) {
    ...
    if (!atag) {
        ...
        if (!atag)that._log("&nbsp;XML not valid"); else that._log("&nbsp;Incorrect content type - need to be text/xml")
    }
}, function() {
});
